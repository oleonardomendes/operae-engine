...
interface Props {
  ...
  freteGratis?: boolean
  kitItens?: Array<{ text: string }>
  kitBonus?: { text: string; sub: string } | null
}
...
export default function CheckoutForm({
  kitNome, kitPreco, backUrls, onFreteSelected, onEnderecoComplete, produtosParaFrete, freteGratis, kitItens, kitBonus,
}: Props) {
  ...
  // Endereço salvo (usuários logados)
  const [enderecoSalvo, setEnderecoSalvo] = useState<any>(null)
  const [cpfSalvo, setCpfSalvo] = useState('')
  const [usarEnderecoSalvo, setUsarEnderecoSalvo] = useState<boolean | null>(null)
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (!guest && user) {
        ...
        if (ultimoPedido?.endereco) {
          const end = typeof ultimoPedido.endereco === 'string'
            ? JSON.parse(ultimoPedido.endereco)
            : ultimoPedido.endereco
          setEnderecoSalvo(end)
          setCpfSalvo(ultimoPedido.cpf || '')
        }
      }
    })
  }, [])
  ...
  async function handleCalcularFrete() {
    if (freteGratis) {
      ...
      const freteGratisOption: FreteOpcao = {
        id: 0,
        nome: 'Frete Grátis',
        empresa: '',
        preco: 0,
      ...
      setFreteSelected(freteGratisOption)
      await handleIrPagamento(freteGratisOption)
      return
    }
    if (!validarStep1()) return
  ...
  async function handleIrPagamento(freteOverride?: FreteOpcao) {
    const frete = freteOverride ?? freteSelected
    if (!frete) return
    ...
    try {
      const res = await fetch('/api/mp/create-preference', {
        ...
        body: JSON.stringify({
          ...
          kitNome,
          kitPreco,
          freteValor: frete.preco,
          freteServico: frete.nome,
          ...(backUrls ? { backUrls } : {}),
        }),
      })
      ...
      if (pid && preferenceId) {
        fetch('/api/pedidos/pre-registro', {
          ...
          body: JSON.stringify({
            ...
            cpf,
            freteValor: frete.preco,
            freteServico: `${frete.empresa || ''} ${frete.nome || ''}`.trim(),
            freteServicoId: frete.id,
            itens: produtosParaFrete,
            total: Number(kitPreco) + Number(frete.preco || 0),
    ...
    try {
      onFreteSelected({ servico: frete.nome, valor: frete.preco, prazo: frete.prazo })
      onEnderecoComplete({
        endereco: { cep, logradouro, numero, complemento, bairro, cidade, estado },
        cpf,
        frete,
  ...
  const progressLabels = freteGratis
    ? (isGuest
        ? ['Seus dados', 'Endereço', 'Pagamento']
        : ['Endereço', 'Pagamento'])
    : (isGuest
  ...
  return (
    <>
      ...
      {step === 0 && (
        <div className="cf-step0-grid">
          <div className="cf-step0-left">
              ...
              <div style={{ marginTop: '8px', marginBottom: '4px' }}>
                ...
                {kitBonus && (
                  <div style={{
                    display: 'flex', alignItems: 'flex-start',
                    gap: '6px', fontSize: '12px',
                    color: 'var(--a600, #c47a00)',
                    fontWeight: '600', marginTop: '4px',
                    lineHeight: '1.4'
                  }}>
                    <span style={{ flexShrink: 0 }}>🎁</span>
                    <span>{kitBonus.text}</span>
                  </div>
                )}
              </div>
            )}
      ...
      {step === 1 && (
        <div className="cf-section">
          ...
          {/* Card de endereço salvo — apenas para logados com pedido anterior */}
          {!isGuest && enderecoSalvo && usarEnderecoSalvo === null && (
            <div style={{ border: '1.5px solid var(--g500)',
              borderRadius: 'var(--r-md)', padding: '16px',
              background: '#f0f7f0', marginBottom: '4px' }}>
              ...
              <div style={{ fontSize: '14px', color: 'var(--dark)',
                ...
                {enderecoSalvo.logradouro}, {enderecoSalvo.numero}
                {enderecoSalvo.complemento ? ` — ${enderecoSalvo.complemento}` : ''}<br/>
                {enderecoSalvo.bairro} · {enderecoSalvo.cidade}/{enderecoSalvo.estado}<br/>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                <button
                  ...
                  onClick={() => {
                    ...
                    setLogradouro(enderecoSalvo.logradouro || '')
                    setNumero(enderecoSalvo.numero || '')
                    setComplemento(enderecoSalvo.complemento || '')
                    setBairro(enderecoSalvo.bairro || '')
                    setCidade(enderecoSalvo.cidade || '')
                    setEstado(enderecoSalvo.estado || '')
                    setCpf(cpfSalvo)
                ...
                <button
                  ...
                  style={{ padding: '10px 16px', border: '1.5px solid var(--border)',
                    borderRadius: '50px' }}
          ...
          {/* Formulário normal — guests, sem endereço salvo, ou escolheu novo endereço */}
          {(isGuest || !enderecoSalvo || usarEnderecoSalvo === false) && (
          <>
          <div className="cf-field">
          ...
          <div style={{ background: '#fff', border: '1px solid var(--border)',
            ...
            {freteSelected && (
              <div style={{ display: 'flex', justifyContent: 'space-between',
                ...
                {freteGratis
                  ? <span style={{ color: 'var(--g500)', fontWeight: '700' }}>🚚 Frete Grátis</span>
            ...
            {freteSelected && (
              <div style={{ fontSize: '12px', color: 'var(--g500)', fontWeight: '600',
                marginTop: '8px', textAlign: 'right' }}>
                ⚡ PIX com 5% de desconto: R$ {(freteGratis
                  ? Number(kitPreco) * 0.95
                  : Number(kitPreco) * 0.95 + Number(freteSelected.preco))
                  .toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
            )}
          </div>
        ...
        <div className="cf-section">
          ...
          <div style={{ background: '#fff', border: '1px solid var(--border)',
            ...
            <div style={{ fontSize: '12px', color: 'var(--g500)', fontWeight: '600',
              ...
              ⚡ PIX com 5% de desconto: R$ {(freteGratis
                ? kitPreco * 0.95
                : kitPreco * 0.95 + freteSelected.preco)
                .toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
          </div>
...
const styles = `
  ...
  @media (max-width: 640px) {
    .cf-step0-grid { grid-template-columns: 1fr; }
    .cf-step0-left { position: static; }
  }
`
